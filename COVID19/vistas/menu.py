...
pcrEstablecimiento = pd.read_csv(
...
casos_act_data = data_crec_por_dia[data_crec_por_dia["Fecha"] == "Casos activos"][
    ultima_fecha_cl_r
].sum()
casos_totales_data = data_crec_por_dia[data_crec_por_dia["Fecha"] == "Casos totales"][
    ultima_fecha_cl_r
].sum()
casos_fallecidos_data = data_crec_por_dia[data_crec_por_dia["Fecha"] == "Fallecidos"][
    ultima_fecha_cl_r
].sum()
casos_recuperados_data = data_crec_por_dia[
    data_crec_por_dia["Fecha"] == "Casos recuperados por FIS"
][ultima_fecha_cl_r].sum()
...
def grafico_Update_Dropdown_chile(data_crec_por_dia, pcrEstablecimiento):
    ...
    # Add Traces
    casos_totales_df = pd.DataFrame(
        {
            "fecha": fecha_casos_totales,
            "casos": data_crec_por_dia[
                data_crec_por_dia["Fecha"] == "Casos nuevos con sintomas"
            ]
            .iloc[0, 1:]
            .values,
        }
    ...
    fallecidos_totales_df = pd.DataFrame(
        {
            "fecha": fecha_casos_totales,
            "casos": data_crec_por_dia[
                data_crec_por_dia["Fecha"] == "Casos nuevos sin sintomas"
            ]
            .iloc[0, 1:]
            .values,
        }
    ...
    casos_nuevos_totales_df = pd.DataFrame(
        {
            "fecha": fecha_casos_totales,
            "casos": data_crec_por_dia[
                data_crec_por_dia["Fecha"] == "Casos nuevos totales"
            ]
            .iloc[0, 1:]
            .values,
        }
    ...
    pcr_ultimo_dia = pd.DataFrame(
        {
            "fecha": fecha_pcr,
            "Total": pcrEstablecimiento[
                pcrEstablecimiento["Establecimiento"] == "Total informados ultimo dia"
            ]
            .iloc[0, 2:]
            .values,
        }
    ...
    fig.add_trace(
        go.Scatter(
            x=casos_nuevos_totales_df.fecha,
                    ...
                    [
                        dict(
                            ...
                            args=[
                                ...
                                {"title": "Casos Diarios: Totales", "annotations": []},
                            ],
                        ),
                        dict(
                            label="Con sintomas",
                        ...
                        dict(
                            label="Casos Diarios VS Examenes PCR",
                            method="update",
                            args=[
...
def menu(request):
    ...
    # Grafico4
    data_sintomas = (
        data_chile_r[data_chile_r["Fecha"] == "Casos nuevos con sintomas"]
        .iloc[0, 1:]
        .sum()
    )
    data_sin_sintomas = (
        data_chile_r[data_chile_r["Fecha"] == "Casos nuevos sin sintomas"]
        .iloc[0, 1:]
        .sum()
    )
    data_casos_por_sintomas = pd.DataFrame(
        {
            "Tipo": ["Casos nuevos con sintomas", "Casos nuevos sin sintomas"],
            "Numero de casos": [data_sintomas, data_sin_sintomas],
        }
    )
