...
casos_act_data = data_chile_r[data_chile_r["Fecha"] == "Casos activos"][
    ultima_fecha_cl_r
].sum()


# recuperados
num_recuFIS_data = data_crec_por_dia[
...
def examenes_pcr(request):
    ...
    fig = px.bar(
        exa_pcr.sort_values("Total"), x="Region", y="Total", title=titulo, text="Total"
    )
